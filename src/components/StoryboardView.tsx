...
const StoryboardView = ({ data, onRegenerateShot }: StoryboardViewProps) => {
  ...
  const handleSaveEdit = (id: string) => {
    ...
    setEditingId(null);
    showSuccess("Regenerating shot with new description...");
  };
  ...
  return (
    <div className="bg-[#1A1A1A] text-white p-8 rounded-xl shadow-2xl overflow-x-auto">
      <div className="flex flex-col items-center mb-10 border-b border-white/10 pb-6">
        <h1 className="text-4xl font-bold tracking-tighter uppercase mb-2">Production Blueprint</h1>
                ...
                <div className="flex flex-col gap-3">
                  <div className="relative">
                    ...
                    <button 
                      ...
                      className="absolute top-2 right-2 bg-black/60 hover:bg-black/80 p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                  ...
                  {editingId === row.id ? (
                    <div className="flex gap-2">
                      <Input 
                        ...
                        className="bg-white/10 border-white/20 text-xs h-8"
                      />
                      <Button size="icon" variant="ghost" className="h-8 w-8 text-green-500" onClick={() => handleSaveEdit(row.id)}>
                        <Check size={14} />
                      </Button>
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-xs text-white/70 italic flex-1">{row.visual}</p>
                      <button 
